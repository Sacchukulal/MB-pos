...
use crate::delivery::{DeliveryEdit, board_on, record_handback_on, save_delivery_on, set_rider_on};
...
fn a_shop(scratch: &Scratch, name: &str) -> App {
    ...
    // One item, at 5%, so the delivery charge's own rate is visibly a different figure from the
    // food's.
    db.transaction(|tx| {
...
/// One delivery order, billed and settled in cash through the real path — which is what a shop
/// that takes the money at the counter and sends the food out does, and the case the drawer
/// gets wrong.
fn a_delivery_paid_in_cash(app: &App) -> (String, Money, mb_core::Bill) {
    ...
    .expect("delivery");
    crate::ipc::cart_add_on(app, "itm_dosa".to_owned(), Some("2".to_owned()), None).expect("added");
    let bill = app
...
// The charge, the rider and the reconciliation.

/// A delivery charge is taxed on its OWN rate, and the cash that goes out with a rider
/// reconciles against what they hand back.
#[test]
fn a_delivery_is_taxed_on_its_own_rate_and_the_cod_cash_reconciles_against_the_handback() {
    ...
    // A ₹40 delivery charge at 5%, while the food is at whatever the menu says — the point is
    ...
    // And it reconciles. Nothing outstanding, and the drawer expects the whole day's cash
    // again.
    assert_eq!(carrying(&app), Money::ZERO);
...
// A failure is a state.

/// A delivery that did not arrive is a STATE with a reason, and the money is still accounted
/// for: nobody paid, so nobody is carrying it, and the bill is still owed.
#[test]
fn a_failed_delivery_is_a_state_with_a_reason_and_the_money_is_still_accounted_for() {
    ...
    assert_eq!(
        row.collect.paise, row.total.paise,
        "the food is unpaid and the board still says so"
    );
    // And the figure is a real one.
    assert!(
        row.total.paise > 0,
    ...
    // Nobody collected anything, so nobody is carrying anything — and the drawer is untouched
    ...
    // And it cannot be quietly turned into a success.
...
/// Anybody signed in may LOOK at the board.
#[test]
fn reading_the_board_is_open_and_moving_a_delivery_is_not() {
    ...
    let refused = record_handback_on(
        &app,
        "staff_kumar".to_owned(),
        "1".to_owned(),
        String::new(),
