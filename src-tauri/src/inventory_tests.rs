//! The stock book, driven end to end.
...
use crate::inventory::{
    MaterialEdit, MovementEdit, PackEdit, RecipeEdit, RecipeLineEdit, buy_list_text_on,
    inventory_on, rebuild_balances_on, recipe_on, record_movement_on, save_material_on,
    save_recipe_on,
};
...
fn a_shop(scratch: &Scratch) -> App {
    ...
    // The stock book is behind `Feature::Inventory`.
...
fn a_pack_is_typed_once_and_read_back_everywhere() {
    ...
    // The reorder level was typed as "1 bag" and comes back as one bag, not as 25,000 g and not
    // as 25 kg.
    assert_eq!(material.reorder_level, "1 bag", "{material:?}");
    assert_eq!(material.reorder_qty, "2 bag");
    assert_eq!(material.base_unit, "g");
    // Kg came free with the dimension; bag is the shop's own.
    ...
    // Nothing has ever been bought, so there is no cost and the screen says so rather than
    // showing ₹0.00.
...
/// Buy two bags, sell forty plates, and read the shelf back in the words a person would use.
#[test]
fn the_whole_sequence_a_shopkeeper_performs() {
    ...
    // Nothing has been priced, so the cost is zero AND the screen names the material rather
    // than quietly presenting ₹0.00 as a food cost.
    assert_eq!(view.cost.paise, 0);
    assert_eq!(view.unpriced.len(), 1);
    assert!(view.unpriced[0].contains("Rice"), "{:?}", view.unpriced);

    // Two bags in, at ₹1,500 a bag — a price typed per PACK, which is what a shop knows.
    record_movement_on(
        &app,
        MovementEdit {
            material_id: "mat_rice".to_owned(),
    ...
    assert!(
        !material.is_low,
        "two bags is not low when the level is one"
    );
    ...
    let material = view
        ...
        .expect("there");
    assert_eq!(
        material.on_hand, "1.712 bag",
        "42,800 g read the way a person would"
    );
...
/// The buy list counts in the pack you buy in, and reads as a sentence you can send to
/// somebody.
#[test]
fn the_buy_list_is_a_message_a_person_can_send() {
    ...
    assert_eq!(
        view.buy_list[0].lines[0].line,
        "Rice — have 12 kg, buy 2 bag"
    );
...
fn the_screen_notices_when_a_balance_stops_matching_its_movements() {
    ...
    assert_eq!(
        view.cache_warning, "",
        "a fresh shop has nothing to warn about"
    );
    ...
    let view = inventory_on(&app, None).expect("the screen");
    assert!(
        view.cache_warning.contains("1 material"),
        "{}",
        view.cache_warning
    );
    assert!(
        view.cache_warning.contains("Rebuild"),
        "{}",
        view.cache_warning
    );
...
/// A wastage entry is valued and reasoned, and is the report that catches theft.
#[test]
fn wastage_is_recorded_with_a_reason_and_a_value() {
    ...
    let reason = view
        .wastage_reasons
        .first()
        .expect("a starting list")
        .clone();
    assert!(
        !view.wastage_reasons.is_empty(),
        "a shop starts with wastage reasons"
    );
...
fn demo_stock() {
    ...
    // Loudly, not `let _ =`: a demo seeded on top of the last one doubles every figure, which
    // cost ten minutes of reading numbers that were exactly twice what they should have been.
    ...
    // The stock screen is behind `Feature::Inventory`, so the demo starts a trial — rooted at
    // the demo's OWN folder and against the machine id that folder resolves to, so the
    // `licence.json` this writes is the one the running app reads when `APPDATA` points here.
    ...
    let material =
        |id: &str, name: &str, dimension: &str, pack: Option<(&str, &str, &str)>, buy: &str| {
            MaterialEdit {
                id: id.to_owned(),
                name: name.to_owned(),
    ...
    // A made material with a batch yield, and two dishes that use it.
    save_recipe_on(
        &app,
        RecipeEdit {
            owner_kind: "material".to_owned(),
            ...
            lines: vec![
                ...
                RecipeLineEdit {
                    ...
                    qty: "1".to_owned(),
                    unit: "kg".to_owned(),
                    // Peeled, so 90% of what is issued reaches the pot.
                    yield_percent: 90,
                },
            ],
        },
    ...
    // Sell some of each, so the ledger has sales, an automatic production and a material that
    // went below zero.
    for n in 0..12 {
        sell(&app, "itm_dosa", n);
    }
