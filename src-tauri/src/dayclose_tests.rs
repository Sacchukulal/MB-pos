//! Closing the day, driven end to end.
...
/// One cash sale through the real billing path, so the drawer figure is a figure the product
...
fn a_drawer_that_matches_closes_without_a_reason() {
    ...
    // Nothing counted yet, so the whole drawer reads as missing — and it says so in words
    // rather than as a minus sign.
    ...
    assert!(
        closed.closed_says.contains("Closed"),
        "{}",
        closed.closed_says
    );
}
...
fn a_short_drawer_cannot_be_closed_without_saying_why() {
    ...
    // The refusal IS the feature, so the refusal is the sentence.
...
fn a_closed_day_refuses_a_void_until_somebody_opens_it_again() {
    ...
    let opened =
        reopen_on(&app, "the 8 o'clock bill was on the wrong table".to_owned()).expect("reopened");
...
/// The float carries forward, and tomorrow needs to know nothing about it: the close writes a
/// `float` movement dated tomorrow, which is exactly what `cash_position` already reads.
#[test]
fn the_float_left_in_the_drawer_is_tomorrows_opening() {
    ...
    let tomorrow = app
        .with_shop(|shop| {
            shop.db
                .read_transaction(|tx| Repos::new(tx).money().cash_position(OUTLET, today.next()))
...
// The slip actually reaches the queue.
...
fn the_handover_report_names_the_shift_the_person_and_the_difference() {
    ...
    // Reports are behind the licence gate; the refusal itself is `licence_tests`' business, not
    // this one's.
