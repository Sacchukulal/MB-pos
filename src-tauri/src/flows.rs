...
/// The kitchen ticket that would print right now — the delta, exactly as the button sends it.
pub fn preview_kitchen_on(
    ...
    let (core, order) = match order_id {
        Some(id) => {
            let order = find_order(app, &OrderId::new(&id))?.ok_or_else(|| {
                UiError::new("preview.no_order", "That order is not on this counter.")
            })?;
    ...
    let document = mb_print::template::kitchen_document(
        ...
        &mb_print::template::KitchenContext {
            ...
            // Not claimed: a preview must not burn a number.
            kot_number: None,
