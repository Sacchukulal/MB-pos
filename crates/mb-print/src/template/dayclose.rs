...
pub struct CountedNote {
    /// "500", "10", "50p" — written by the caller, because what a shop calls a coin is the
    /// shop's language and not this crate's.
    pub label: String,
    pub count: u32,
    pub total: String,
}
...
/// Everything the slip prints.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DayCloseContext<'a> {
    pub store: &'a Store,
    pub day: &'a str,
    /// "Closed at 11:14 pm by Ravi" — one sentence, assembled by the caller which knows the
    /// shop's clock and the person's name.
    pub closed: &'a str,
    ...
    /// "Left in the drawer for tomorrow: 2,000.00", when the shop carries a float.
        ...
        doc.separator(Pattern::Solid)
            ...
            .push(Block::Columns {
                columns: vec![
                    ...
                    // Be a column of its own — `wrap` drops leading spaces, and that is how
                    // "2Paneer Butter Masala" reached a kitchen for eleven prompts.
...
mod tests {
    ...
    fn a_slip(variance: &str, reason: Option<&str>) -> String {
        ...
        let takings = [
            SlipLine {
                label: "Bills (48)".to_owned(),
                amount: "24,120.00".to_owned(),
            },
            SlipLine {
                label: "Voided (2)".to_owned(),
                amount: "-620.00".to_owned(),
            },
        ];
        let drawer = [
            SlipLine {
                label: "Opening float".to_owned(),
                amount: "2,000.00".to_owned(),
            },
            SlipLine {
                label: "Cash sales".to_owned(),
                amount: "9,400.00".to_owned(),
            },
        ];
        let counted = [
            CountedNote {
    ...
    /// The slip carries every figure a person needs to check the day, and the difference is
    /// words rather than a signed number.
    #[test]
    fn the_slip_says_what_is_missing_in_words() {
        let text = a_slip(
            "SHORT BY 340.00",
            Some("paid the vegetable man from the drawer"),
        );
    ...
    /// The denomination grid keeps its gutter.
    #[test]
    ...
    /// The narrowest paper a shop can own still holds the slip.
    #[test]
