...
pub fn test_document(printer: &PrinterConfig, store: Option<&Store>) -> Document {
    ...
    // Drawn by the layout, at the width it measured.
    ...
    // What the offset currently is, in both units.
    ...
    // A setting that silently did less than it was asked is how somebody spends an evening
    // nudging a number that stopped moving three steps ago.
    if let Some(clamped) = clamp_warning(&doc) {
        doc.text(&clamped, Style::BOLD, Align::Left);
    }
    doc.separator(Pattern::Dashed);

    // A real-looking bill, so the shape on the paper is the shape a customer will see.
    doc.text("Sample bill", Style::BOLD, Align::Left);
    doc.row(
        "Masala Dosa x2",
        Money::from_paise(24_000).to_plain_string(),
        Style::NORMAL,
    );
    doc.row(
    ...
    doc.row(
        "Water 1L x3",
        Money::from_paise(6_000).to_plain_string(),
        Style::NORMAL,
    );
    doc.separator(Pattern::Dashed);
    doc.row(
        "Subtotal",
        Money::from_paise(61_500).to_plain_string(),
        Style::NORMAL,
    );
    doc.row(
        "CGST 2.5%",
        Money::from_paise(1_538).to_plain_string(),
        Style::NORMAL,
    );
    doc.row(
        "SGST 2.5%",
    ...
    doc.row(
        "Round off",
        Money::from_paise(24).to_plain_string(),
        Style::NORMAL,
    );
    doc.row(
        "TOTAL",
    ...
    doc.text(
        "If this looks right, printing works.",
        Style::NORMAL,
        Align::Centre,
    );
    doc.spacer(2);
...
pub fn ruler_numbers(columns: usize) -> String {
    ...
    for i in 0..columns {
        if i % 10 == 0 {
            // 0, 1, 2 … for columns 0, 10, 20. One character, so it cannot push the ruler out
            // of step with the marks above it.
...
fn describe_mm(mm: i32, printer: &PrinterConfig) -> String {
    ...
    format!("{mm:+} mm {direction} ({} characters)", columns.abs())
}
