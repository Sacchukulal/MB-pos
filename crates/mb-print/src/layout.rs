...
// The workspace denies integer division because of D7 — nothing in the MONEY path may be
// silently lossy.
#![allow(
...
/// How thick a drawn rule is, in dots, for each pattern.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct Rule {
    /// Dots of ink, top to bottom, in each stroke.
    pub thickness: u32,
    /// Dots of ink before a gap.
...
impl Rule {
    ...
    /// Air above and below a rule, in dots.
    pub const AIR: u32 = 3;
...
pub enum LaidContent {
    ...
    /// A logo and the shop's name side by side.
    Band {
        image: Vec<u8>,
        image_left: u32,
        image_top: u32,
    ...
    /// A scannable code, and the digits under it.
    Barcode {
        payload: String,
        human_readable: bool,
        align: Align,
...
/// Where one aligned run sits on a line, measured in characters of that line's own size.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct Segment {
...
pub struct LaidLine {
    ...
    pub style: Style,
    /// Dots from the left edge of the paper, offset already applied.
    pub indent_dots: u32,
    /// Dots of paper this line spends, top to bottom.
    pub row_dots: u32,
...
/// Something the layout had to do to make the document fit, that a person might want to know
/// about.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
pub enum Note {
    ...
    /// A row's label wrapped so its amount could stay whole.
    LabelWrapped {
        label: String,
    },
    /// A logo that could not be read, so the letterhead printed as text across the whole paper.
    LogoUnreadable {
        reason: String,
    },
}
...
pub struct Laid {
    ...
    pub notes: Vec<Note>,
    /// One character of the body size, in dots.
...
pub fn layout_for(doc: &Document, metrics: &Metrics) -> Result<Laid, PrintError> {
    ...
    let top_dots = u32::try_from(top).unwrap_or(0) * crate::paper::DOTS_PER_MM;
    if top_dots > 0 {
        lines.push(LaidLine {
    ...
    // The usable width is what is left after the offset, so a shifted document still fits
    // rather than falling off the right edge.
    let usable = dots.saturating_sub(indent_dots).max(base_advance);

    for block in &doc.blocks {
        lay_block(block, usable, indent_dots, metrics, &mut lines, &mut notes)?;
    }
...
fn clamp_offset(asked_mm: i32, dots: u32, advance: u32, notes: &mut Vec<Note>) -> u32 {
    ...
    let asked_dots = asked_mm.saturating_mul(i32::try_from(crate::paper::DOTS_PER_MM).unwrap_or(8));
    ...
    // Never let the offset push content off the paper.
...
fn lay_block(
    ...
    match block {
        ...
        Block::Text {
            ...
            // The last resort, and only that: a single word wider than the paper cannot be
            // drawn at this size at all.
...
#[expect(
    ...
    reason = "one band, and every one of these is a position it needs"
)]
fn lay_band(
    image: &[u8],
    side: Align,
    ...
    let text_width = usable
        .saturating_sub(image_width)
        .max(metrics.body().advance);

    // The header is read, not the picture.
    let Some((source_w, source_h)) = Monochrome::size(image).filter(|(w, _)| *w > 0) else {
        ...
        // A shop with a corrupt logo still gets its bill — with the letterhead across the whole
        // paper, which is what it would have been without a logo at all.
        for line in text {
            lay_block(
                &Block::Text {
...
/// The fixed columns plus one character of fill — the least a table can be drawn in at all.
...
fn fit_at_least(
    ...
    for rung in Style::LADDER.iter().rev() {
        ...
        if metrics.size(candidate).chars_across(usable) >= needed {
            notes.push(Note::ScaleCapped { asked, used: *rung });
            return candidate;
        }
    }
    // Even the smallest cannot hold it.
...
fn fit_row(
    ...
    // At least one space between the label and the amount, always: a label that touches its
    // amount reads as one number.
    ...
    if left_lines.len() > 1 {
        ...
        for line in &left_lines[1..] {
            // Indented by two, so a continuation reads as part of the line above rather than as
            // a new item.
            out.push((
                format!("  {line}"),
...
fn fit_columns(columns: &[Column], width: usize) -> Vec<usize> {
    ...
    // Whatever is still missing or spare lands on the widest column, so the sum is exact rather
    // than nearly right.
...
fn lay_row(columns: &[Column], widths: &[usize], cells: &[String]) -> Vec<(String, Vec<Segment>)> {
    ...
    for row in 0..height {
        ...
        // The trim is what the text sink wants — trailing spaces are paper.
        out.push((line.trim_end().to_owned(), segments));
    }
    out
}

/// Wrap on spaces, and break a word that is longer than the whole line rather than letting it
/// overflow.
...
mod tests {
    ...
    fn the_money_wins_and_the_label_wraps() {
        ...
        // And the boxes say where the amount is, so a proportional face puts it against the
        // same right edge the padding does.
        ...
        // A continuation line is all label and no amount — the case a "find the last run of
        // spaces" guess got wrong.
    ...
    /// Every size on the ladder produces a different line.
    #[test]
    fn every_size_changes_what_is_laid_out() {
        ...
        for cap in Style::LADDER {
            ...
            assert!(
                !laid.was_capped(),
                "size {cap} was reduced for four letters"
            );
            assert!(
    ...
    /// A size is only reduced when the block cannot be drawn at all — a table whose fixed
    /// columns already fill the paper.
    #[test]
    fn a_table_that_cannot_be_drawn_at_all_comes_down_a_rung() {
        ...
        assert!(
            laid.was_capped(),
            "29 fixed columns cannot fit 58 mm at that size"
        );
        // And it came down to a rung a shop could have chosen, not to an arbitrary number.
    ...
    /// The logo band: 30 % picture, 70 % text, and the shorter of the two centred against the
    /// taller.
    #[test]
    fn a_band_puts_the_logo_beside_the_name() {
        ...
        for line in lines {
            assert_eq!(
                ...
                "the text does not start after the logo"
            );
            assert_eq!(line.width, m.dots() - image_width);
        }
    }

    /// A logo that will not read is still a bill.
    #[test]
