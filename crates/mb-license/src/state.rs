...
impl LicenceFile {
    ...
    /// Write it, atomically — through a temporary file and a rename, so a power cut mid-save
    /// cannot leave half a JSON object where the licence was.
...
/// By hand, because `Arc<dyn Cloud>` is not `Debug` and never will be — the same treatment
/// `magic-bill`'s `Network` gets.
...
impl Licensing {
    ...
    pub fn entitlement(&self, now: Timestamp, today: BusinessDay) -> Entitlement {
        ...
        // Is this still the machine the licence is for?
    ...
    /// Move a licence onto this machine.
    pub fn transfer(
        &mut self,
        key: &str,
        now: Timestamp,
...
mod tests {
    ...
    fn scratch(label: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("mb-licence-{}-{label}", std::process::id()));
    ...
    fn a_first_run_is_unactivated_and_still_has_limits() {
        ...
        assert_eq!(
            ...
            Timestamp::EPOCH,
            "it has spoken to nothing"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// "Last checked" means the last time we reached the cloud, not the last time we made a
    ...
    /// The cooldown, and the days left are in the error.
    #[test]
    ...
    fn a_cloud_that_never_answers_does_not_hang_the_counter() {
        ...
        // And the shop is still entitled, from the cache.
        assert_eq!(
            licensing
    ...
    /// A tampered file is refused, and the counter falls back to unactivated — which still
    /// bills.
    #[test]
    ...
    fn wrong_emergency_codes_are_rate_limited() {
        ...
        // The next one is refused for a different reason, and the reason carries the wait.
