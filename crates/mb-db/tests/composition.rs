...
/// Half and full are two prices on one item, and each is its own thing to cook.
#[test]
fn an_item_can_have_sizes() {
    ...
    let sizes = db
        .transaction(|tx| {
            Repos::new(tx)
                .composition()
                .variants_of(&ItemId::new("itm_dosa"))
        })
        .expect("variants");
    ...
    // A half dosa is not a discounted dosa: it carries its own price, so the rate summary and
    // the margin are both right.
    assert_ne!(sizes[0].unit_price, sizes[1].unit_price);
}

/// The group rules are enforced, and in Rust.
#[test]
...
/// A group nobody could satisfy is refused when it is saved, not when a cashier meets it at the
/// counter.
#[test]
fn an_impossible_group_is_refused_at_the_moment_it_is_written() {
    ...
    assert!(
        refused.to_string().contains("nobody can satisfy"),
        "{refused}"
    );
}

/// 2, round trip — including a negative delta, which is a real line on a real menu.
#[test]
...
fn a_combo_apportions_its_price_across_what_is_in_it() {
    ...
    // The fixture's dosa is 5% at 120.00 and its water is 18% at 20.00 — the exact case a combo
    // gets wrong.
    let standalone = [
    ...
    // The shares are proportions of the combo price, and the dosa — worth six times the water —
    // takes much the larger slice.
    let dosa = lunch
        .components
        .iter()
    ...
    assert!(
        (8_500..=8_600).contains(&dosa.share_bp),
        "{}",
        dosa.share_bp
    );

    // And the money itself is exact — recomputed from live prices at the moment of sale, which
    // is the half `share_bp` deliberately does not do.
...
/// A combo whose parts have no price of their own is refused rather than split arbitrarily — an
/// invented split is the wrong rate on somebody's return.
#[test]
fn a_combo_of_priceless_things_is_refused() {
    ...
    let refused = db
        .transaction(|tx| {
            Repos::new(tx).composition().save_combo(
                ...
                // Nothing standalone: the caller did not say what the dosa is worth on its own.
                &[],
                at(1),
            )
        })
    ...
    assert!(
        refused.to_string().contains("price of its own"),
        "{refused}"
    );
}
...
fn one_bad_row_refuses_the_whole_file_by_line_number() {
    ...
    assert!(
        plan.refused[2].1.contains("no name"),
        "{}",
        plan.refused[2].1
    );

    assert!(
        plan.summary().contains("nothing will be imported"),
        "{}",
        plan.summary()
    );
...
/// Export then import must be a no-op.
#[test]
