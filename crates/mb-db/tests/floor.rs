//! The table master, the floor plan and the occupancy line.
...
fn a_table_that_would_print_an_existing_name_is_refused_by_name() {
    ...
    db.transaction(|tx| {
        ...
        repos
            .floor()
            .save_table(OUTLET, &table("tbl_ac_1", Some("sec_ac"), "1"), at(2))
    })
    .expect("AC 1 created");
...
/// The corollary, and the case the rule this replaced got wrong: the most ordinary layout in
...
fn the_same_number_in_two_rooms_is_allowed() {
    ...
    db.transaction(|tx| {
        ...
        repos
            .floor()
            .save_table(OUTLET, &table("tbl_g_1", Some("sec_garden"), "1"), at(3))
    })
    .expect("both rooms may have a table 1");
}

/// All or nothing. A shop that asked for 1-20 and silently got 14 has to find the missing six
/// by counting.
#[test]
...
/// A table with an open order cannot be hidden or deleted, and the refusal says which order.
#[test]
fn a_table_with_an_open_order_cannot_be_hidden_or_deleted() {
    ...
    assert!(
        !built.orders.is_empty(),
        "the fixture leaves an order on a table"
    );
    ...
    let hidden = db.transaction(|tx| {
        Repos::new(tx)
            .floor()
            .set_active(OUTLET, &busy, false, at(9))
    });
...
/// A table that has taken an order can only ever be hidden — and it is told that, with the
/// number, rather than being handed a foreign-key error.
#[test]
fn a_used_table_can_be_hidden_and_never_deleted() {
    ...
    db.transaction(|tx| {
        Repos::new(tx).floor().save_table(
            OUTLET,
            &table("tbl_typo", Some("sec_hall"), "Typo"),
            at(1),
        )
    })
    .expect("added");
    db.transaction(|tx| {
        Repos::new(tx)
            .floor()
            .delete_table(OUTLET, &TableId::new("tbl_typo"), at(2))
    })
    ...
    // Settle or cancel whatever is open on it first, so what is left is history rather than an
    // open order — that is the case being tested.
    let said = db
...
/// Two tables cannot share a square.
#[test]
fn a_floor_plan_cell_holds_one_table() {
    ...
    db.transaction(|tx| {
        Repos::new(tx)
            .floor()
            .place(OUTLET, &TableId::new("tbl_1"), Some((3, 3)), at(1))
    })
    .expect("placed");
    ...
    let said = db
        .transaction(|tx| {
            Repos::new(tx)
                .floor()
                .place(OUTLET, &TableId::new("tbl_2"), Some((3, 3)), at(2))
        })
        .expect_err("occupied")
        .to_string();
    ...
    assert!(
        db.transaction(|tx| {
            Repos::new(tx)
                .floor()
                .place(OUTLET, &TableId::new("tbl_2"), Some((99, 0)), at(3))
        })
        .is_err()
    );
    ...
    let one = tables
        .iter()
        .find(|t| t.id.as_str() == "tbl_1")
        .expect("table 1");
    assert_eq!(one.pos, Some((3, 3)));
}

/// A table added after a layout exists lands somewhere visible.
#[test]
fn a_new_table_finds_an_empty_square() {
    ...
    assert!(
        !taken.contains(&cell),
        "the square offered is genuinely empty"
    );
...
fn occupancy_reconciles_with_the_open_orders() {
    ...
    let (numbers, open_tables, active_tables) = db
        .transaction(|tx| {
            let repos = Repos::new(tx);
            let open = repos.orders().list_open(OUTLET)?;
            let day = open.first().map_or_else(
                || BusinessDay::from_days_since_epoch(0),
                |o| o.core().business_day,
            );
    ...
    assert_eq!(
        usize::try_from(numbers.tables).expect("a count"),
        active_tables
    );
    assert!(numbers.busy <= numbers.tables);
}

/// A merged-away order is recorded, not deleted, and the link is a column so a report can tell
/// a merge from a walkout.
#[test]
