...
use mb_core::{
    BillInput, BusinessDay, Cart, Dimension, ItemId, ItemSnapshot, MaterialId, ModifierId, Money,
    OrderId, OrderType, Payment, PaymentMode, PlaceOfSupply, Qty, Registration, RoundingMode,
    Settlement, StaffId, TaxRate, TaxSpec, Timestamp, UnitCost, compute_bill,
};
...
fn t1_two_bags_in_forty_plates_out_and_the_row_does_not_move() {
    ...
    // The shop corrects the bag to 26 kg.
    let mut bigger = rice();
    ...
    let rows = db
        .transaction(|tx| {
            Repos::new(tx)
                .stock()
                .movements(OUTLET, Some(&material("mat_rice")), None, None, 50)
        })
        .expect("movements");
    let purchase = rows
        .iter()
    ...
    assert_eq!(
        balance(&db, "mat_rice"),
        grams(42_800),
        "and neither did the balance"
    );
}

/// A dish made of a gravy made of materials, deducting both levels with the batch yield
/// pro-rated.
#[test]
fn t2_multi_level_recipe_deducts_through_every_level() {
    ...
    save(
        &db,
        &[
            plain("mat_gravy", "Gravy base"),
            plain("mat_tomato", "Tomato"),
            plain("mat_onion", "Onion"),
        ],
    );
    ...
    // The gravy was made and immediately used, so it nets to nothing.
    ...
    let rows = db
        .transaction(|tx| {
            Repos::new(tx)
                .stock()
                .movements(OUTLET, None, None, None, 100)
        })
        .expect("movements");
    assert!(
        rows.iter()
            .any(|r| r.kind == MovementKind::ProductionIn && r.was_automatic),
    ...
    // The made material's cost came from its inputs.
    let made = rows
        .iter()
        .find(|r| r.kind == MovementKind::ProductionIn)
        .expect("made");
    assert_eq!(made.unit_cost, UnitCost::from_paise_per_thousand(3_750));

    // An input row names what it fed.
    let input = rows
        .iter()
    ...
    // The FIRST sale of a made material is valued, which it was not until the rows were written
    // in the order the kitchen did them.
    let sold_gravy = rows
        .iter()
    ...
    // And the whole thing is exact, not pro-rated through a rounded batch fraction.
    ...
    db2.transaction(|tx| {
        ...
        stock.save_recipe(
            OUTLET,
            &recipe_for("itm_dosa", vec![("mat_gravy", 150)]),
            at(1),
        )
    })
    .expect("recipes");
    ...
    // 150 g of gravy is 0.0375 of a batch, so 112.5 g of tomato — not the 114 g a batch
    // fraction rounded to thousandths would have taken.
    assert_eq!(
...
/// A recipe that eats itself is refused at save, by name.
#[test]
fn t3_a_loop_is_refused_and_the_message_names_it() {
    ...
    println!("\n  {said}");
    assert!(
        said.contains("Powder"),
        "the message does not name the loop: {said}"
    );
    assert!(
        said.contains("Paste"),
        "the message does not name the loop: {said}"
...
fn t5_a_void_puts_back_what_was_actually_taken() {
    ...
    db.transaction(|tx| {
        Repos::new(tx)
            .stock()
            .reverse_for_bill(OUTLET, &settled.core.id, at(21), day(), None)
    })
    .expect("reversed again");
    assert_eq!(
        balance(&db, "mat_rice"),
        grams(10_000),
        "a second void doubled the stock"
    );
}
...
fn t6_nothing_in_the_stock_book_can_refuse_a_bill() {
    ...
    // A recipe still reaching for a material the shop has RETIRED.
    ...
    assert!(
        one.bill.grand_total.is_positive(),
        "the bill did not settle"
    );
    assert_eq!(
        balance(&db, "mat_ghee"),
        grams(-50),
        "a retired material still moved"
    );

    // A shelf that goes below zero.
    put_recipe(&db, &recipe_for("itm_dosa", vec![("mat_rice", 1_000)]));
    let two = settle_one(&db, "ord_t6_2", dosa(), 3, vec![]);
    assert!(
        two.bill.grand_total.is_positive(),
        "the bill did not settle"
    );
    assert_eq!(
        balance(&db, "mat_rice"),
        grams(-3_000),
        "a negative balance is information"
    );
    ...
    assert!(
        three.bill.grand_total.is_positive(),
        "the bill did not settle"
    );

    // An ad-hoc line with no item id at all.
    let adhoc = ItemSnapshot::new(
        ItemId::new(""),
        "Special",
    ...
    assert!(
        four.bill.grand_total.is_positive(),
        "the bill did not settle"
    );
    ...
    // The ad-hoc line raised NOTHING.
    assert!(
    ...
    assert!(
        problems
            .iter()
            .all(|p| p.sentence.len() > 30 && p.sentence.ends_with('.'))
    );
}

/// The derived balance equals the sum of the ledger, over a generated month of purchases,
/// sales, wastage and adjustments.
#[test]
fn t7_the_cache_always_agrees_with_the_ledger() {
    ...
    // A month. The quantities are arbitrary but the mixture is not: every kind that can move
    // stock outside a bill is in here.
    let kinds = [
        MovementKind::Purchase,
    ...
    for d in 0..30 {
        for (m, material_row) in materials.iter().enumerate() {
            for (k, kind) in kinds.iter().enumerate() {
                n += 1;
                let sign = if matches!(kind, MovementKind::Wastage) {
                    -1
                } else {
                    1
                };
    ...
    let cached = db
        .transaction(|tx| Repos::new(tx).stock().balances(OUTLET))
        .expect("cached");
    let corrected = db
...
/// A variant recipe and a modifier recipe each deduct their own.
#[test]
fn t8_variants_and_modifiers_deduct_their_own_amounts() {
    ...
    save(
        &db,
        &[plain("mat_batter", "Batter"), plain("mat_cheese", "Cheese")],
    );
    ...
    assert_eq!(
        balance(&db, "mat_batter"),
        grams(10_000 - 240),
        "the half plate used the full recipe"
    );
    assert_eq!(
        balance(&db, "mat_cheese"),
        grams(5_000 - 60),
        "the modifier did not deduct"
    );
}

/// Theoretical against actual, and the "never counted" that stops it being a confident lie.
#[test]
fn t9_the_variance_report_finds_a_seeded_wastage_and_admits_what_it_does_not_know() {
    ...
    receive(
        ...
        grams(100),
        "kg",
        UnitCost::from_paise_per_thousand(6_000),
        5,
    );
    ...
    let rice_row = rows
        .iter()
        .find(|r| r.material == material("mat_rice"))
        .expect("rice");
    println!(
    ...
    assert_eq!(
        rice_row.variance_value,
        Money::from_paise(1_800),
        "300 g at ₹60/kg"
    );

    // Nobody has counted this, and the row says so rather than implying the figure has been
    // checked against a shelf.
    assert_eq!(
        rice_row.last_counted_at, None,
        "a variance nobody counted must say so"
    );
}

/// The average cost is what actually came in.
#[test]
fn the_cost_of_a_material_is_a_weighted_average_and_not_a_typed_price() {
    ...
    receive(
        ...
        grams(10),
        "kg",
        UnitCost::from_paise_per_thousand(8_000),
        6,
    );
    ...
    let found = db
        .transaction(|tx| {
            Repos::new(tx)
                .stock()
                .material(OUTLET, &material("mat_rice"))
        })
        .expect("read")
        .expect("there");
    assert_eq!(
    ...
    assert!(
        found.cost_changed_at.is_some(),
        "the screen has to be able to say when"
    );
...
/// The buy list, in the pack the shop buys in.
#[test]
fn the_buy_list_counts_in_bags_and_groups_by_where_you_buy_it() {
    ...
    assert_eq!(
        low.len(),
        1,
        "the milk is not low and should not be on the list"
    );
    let rice_row = low[0];
    ...
    let shortfall = units
        .from_base(
            rice_row.material.reorder_qty,
            &rice_row.material.default_purchase_unit(),
        )
        .expect("in bags");
    println!(
        "\n  {} — have {} · buy {} bag(s) from {}",
        rice_row.material.name,
        units.say(rice_row.base_qty),
        shortfall,
        rice_row.material.buy_from
    );
    assert_eq!(shortfall, Qty::from_whole(2).expect("ok"));
}

/// Wastage is valued, because a wastage figure with no rupees on it is one nobody reads.
#[test]
fn wastage_is_valued_at_what_the_material_costs() {
    ...
    let rows = db
        .transaction(|tx| {
            Repos::new(tx)
                .stock()
                .movements(OUTLET, None, None, None, 10)
        })
        .expect("movements");
    let waste = rows
        .iter()
        .find(|r| r.kind == MovementKind::Wastage)
        .expect("there");
    // The caller passed NO cost, and the row is still valued — resolved inside `record`, so no
    // call site can produce a theft report reading ₹0.
    assert_eq!(
        waste.total_cost,
...
fn a_shop_with_no_recipes_leaves_no_trace() {
    ...
    assert!(
        problems.is_empty(),
        "a shop with no recipes was told it has a problem"
    );
...
/// The day close writes a closing figure per material.
#[test]
fn the_day_close_freezes_a_closing_figure_per_material() {
    ...
    receive(
        ...
        grams(5),
        "kg",
        UnitCost::from_paise_per_thousand(12_000),
        6,
    );
    ...
    db.transaction(|tx| Repos::new(tx).stock().close_day(OUTLET, day()))
        .expect("closed again");
...
fn a_material_knows_which_recipes_use_it() {
    ...
    let used = db
        .transaction(|tx| {
            Repos::new(tx)
                .stock()
                .where_used(OUTLET, &material("mat_rice"))
        })
        .expect("used");
...
fn t10_budget_i1_what_deduction_costs_a_settle() {
    ...
    // A shop with recipes: five materials on the dish, one of them a made material with two of
    // its own, which is deeper than most shops go.
    ...
    // The same shop with NO recipes, so the difference is the deduction and not the settle.
    ...
    println!(
        "  the deduction costs     median {:.2} ms · p95 {:.2} ms",
        median - bare_median,
        p95 - bare_p95
    );
    ...
    assert!(
        added < 25.0,
        "the deduction added {added:.1} ms to the p95 settle, over I1's 25 ms ceiling"
    );
}

/// An empty recipe is refused in words, because a recipe with nothing in it would take nothing
/// off the shelf and look like it was working.
#[test]
fn an_empty_recipe_is_refused_in_words() {
    ...
    let refused = db
        ...
        .expect_err("refused");
    assert!(
        refused.to_string().contains("Add at least one material"),
        "{refused}"
    );
}
