...
fn main() {
    ...
    db.transaction(|tx| {
        for sql in [
            ...
            // A merge and a split are invisible above: the merged-away order shows only as
            // `cancelled`, and the new half of a split looks like any other open order.
            ...
            // The credit account, from its three sources — and the balance is deliberately NOT
            // here, because there is no balance column to read: it is the sum of these rows.
            ...
            // The two places a setting is kept, and reading them back is the only proof that
            // the settings screen did anything — the screen saying "Saved" is the thing being
            // tested.
            ...
            "SELECT kind, terminal_id, prefix, pad_width, reset_daily, start, last_issued
               FROM counters",
            // The night's record: what the till expected, what a person counted, the
            // difference, why, and whether the day is sealed.
            ...
            // Which phones this counter serves, and — the column that matters — that a revoked
            // one is still HERE, as a state and not a deletion.
            ...
            // The stock book, and the point of reading it back is that a balance is DERIVED.
            ...
            // The paper, and the four ledgers it moves.
            ...
            // The landed cost is the number this whole module exists to get right, so it is in
            // the dump beside what was typed.
            ...
            // The balance is a SUM and never a column.
            "SELECT s.name,
            ...
            // The frozen book, and the delta it posted.
            ...
            // A row here with no file beside the database is a detectable fact, which is the
            // whole reason the metadata is in the database.
