...
use crate::cart::Cart;
use crate::discount::{self, Discount};
...
pub struct BillInput<'a> {
    ...
    /// P02 replaces this with the round-off MODE setting (nearest / up / down,
    /// scope 1.13). Nearest is implemented here because it is the only one D4
    /// step 7 needs to be provable.
    pub round_off_enabled: bool,
}

impl<'a> BillInput<'a> {
    /// A plain bill: no discount, same-state supply, dine-in, round-off on.
    #[must_use]
    ...
    #[must_use]
    pub fn with_bill_discount(mut self, discount: Discount) -> Self {
    ...
    #[must_use]
    pub fn with_round_off(mut self, enabled: bool) -> Self {
...
pub struct Bill {
    ...
    /// P02 can tell the cashier and P11 can audit it — never silent (D7).
...
pub fn compute_bill(input: BillInput<'_>) -> Result<Bill> {
    ...
    for (line, line_gross) in lines.iter().zip(&gross) {
        let taken = match line.line_discount {
            Some(discount) => discount.compute_on(*line_gross)?.applied,
            None => Money::ZERO,
        };
    ...
    let bill_discount_shares = match input.bill_discount {
        Some(discount) => {
            ...
            let outcome = discount.compute_on(base)?;
    ...
    // ---- Step 5: charges -------------------------------------------------
    //
    // Service, packing and delivery charges, each with its own tax rate, are
    // scope 1.14 and belong to P02. They enter here, as their own lines with
    // their own TaxOutcome, which is why `Bill` already holds a Vec of lines
    // and a TaxSummary rather than a fixed set of totals — adding them will not
    // change any type in this file.
    ...
    // The grand total is built from the lines themselves, not from
    // subtotal − discount + tax. Summing what will actually be printed is what
    // makes requirement 7 ("the printed lines always sum to the printed
    // total") true by construction rather than by coincidence.
    let before_round_off = Money::try_sum(computed.iter().map(|l| l.gross_including_tax))?;

    // ---- Step 7: round-off, on the grand total ONLY ----------------------
    //
    // Audit B8: v1 printed ₹487.35 and Indian counters round to the rupee.
    let round_off = if input.round_off_enabled {
        before_round_off.round_off_adjustment()
    } else {
        Money::ZERO
    };
...
mod tests {
    ...
    fn assert_reconciles(bill: &Bill) {
        let from_lines = Money::try_sum(bill.lines.iter().map(|l| l.gross_including_tax))
            .expect("sums")
            .add(bill.round_off)
            .expect("adds");
        assert_eq!(
    ...
    fn every_generated_bill_reconciles_both_ways() {
        ...
        for case in 0..3_000_u32 {
            ...
            for line in 0..line_count {
                ...
                if rng.below(3) == 0 {
                    ...
                    if let Some(discount) = discount {
                        cart.set_line_discount(index, Some(discount)).expect("sets");
                    }
                }
            }
            ...
            let mut input = BillInput::new(&cart)
                ...
                .with_round_off(rng.below(2) == 0);
            input.bill_discount = bill_discount;
    ...
    fn a_bill_discount_on_a_mixed_rate_bill_taxes_correctly() {
        ...
        let plain = compute_bill(BillInput::new(&cart).with_round_off(false)).expect("computes");
    ...
    fn an_all_complimentary_bill_with_a_discount_does_not_divide_by_zero() {
        ...
        let bill = compute_bill(
            BillInput::new(&cart).with_bill_discount(Discount::percent_bp(1_000).expect("valid")),
        )
        .expect("computes");
    ...
    fn a_bar_bill_keeps_alcohol_out_of_the_gst_return() {
        ...
        let bill = compute_bill(BillInput::new(&cart).with_round_off(false)).expect("computes");
    ...
    fn a_capped_bill_discount_reaches_the_bill() {
        ...
        let bill = compute_bill(
            BillInput::new(&cart)
                .with_round_off(false)
                .with_bill_discount(Discount::amount(rs(500)).expect("valid")),
        )
        .expect("computes");
    ...
    fn round_off_reaches_the_rupee_and_is_recorded_separately() {
        ...
        let rounded = compute_bill(BillInput::new(&cart).with_round_off(true)).expect("computes");
        assert_eq!(rounded.round_off, Money::from_paise(-35));
        assert_eq!(rounded.grand_total, rs(487));
        assert_reconciles(&rounded);

        let unrounded = compute_bill(BillInput::new(&cart).with_round_off(false)).expect("computes");
    ...
    fn both_discounts_apply_in_the_right_order() {
        ...
        let index = cart
            ...
            .expect("adds");
        cart.set_line_discount(index, Some(Discount::amount(rs(100)).expect("valid")))
            .expect("sets");
