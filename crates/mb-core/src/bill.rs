...
pub enum BillError {
    ...
    /// Modifier deltas below the item's own price.
...
pub struct BillInput<'a> {
    ...
    /// Service, packing, delivery — each with its own tax rate.
    pub charges: &'a [Charge],
    pub place_of_supply: PlaceOfSupply,
    /// Who this shop is.
    pub registration: Registration,
    /// What the state half is called here — SGST, or UTGST in a union territory without a
    /// legislature.
...
impl<'a> BillInput<'a> {
    /// A plain bill: no discount, no charges, same-state supply, dine-in, rounded to the
    /// nearest rupee.
    #[must_use]
...
/// One computed line, carrying every intermediate figure the bill was built from — so the
/// receipt, the screen and the GST return all read the same numbers rather than each
/// recomputing them.
...
pub struct Bill {
    ...
    /// Charges before their own tax.
    pub total_charges: Money,
    /// The bill discount could not take everything it was asked for.
    pub bill_discount_capped: bool,
    pub total_taxable: Money,
    pub total_gst: GstAmounts,
    /// State VAT collected on this bill.
    pub total_vat: Vat,
    /// The part of the GST already inside the prices — the inclusive lines and charges.
    #[serde(default)]
    pub gst_included: GstAmounts,
    /// The same split for VAT.
    #[serde(default)]
    pub vat_included: Vat,
    #[serde(default)]
    pub vat_added: Vat,
    /// `total_tax` less `Bill::tax_included` — the tax the customer is being charged on top of
    /// the printed amounts.
    #[serde(default)]
    ...
    /// 8. Recorded as its own figure so the printed lines always sum to the printed total.
    pub round_off: Money,
    pub grand_total: Money,
    pub order_type: OrderType,
    pub place_of_supply: PlaceOfSupply,
    /// Frozen with the bill, for the same reason the place of supply is: a shop that leaves the
    /// composition scheme next year must not have last year's bills reprint with a different
    /// title.
    pub registration: Registration,
    pub state_tax: StateTax,
    pub rounding: RoundingMode,
...
impl Bill {
    /// Every tax on this bill as one figure.
...
fn gst_split(
    ...
    for line in lines
        .iter()
        .filter(|l| l.tax.basis == PriceBasis::Inclusive)
    {
        included = included.add(line.gst)?;
    }
    for charge in charges
        .iter()
        .filter(|c| c.tax.basis == PriceBasis::Inclusive)
    {
...
pub fn compute_bill(input: BillInput<'_>) -> Result<Bill> {
    ...
    // Line gross = effective unit price × quantity.
    ...
    // Bill discount spread across lines, in proportion to net.
    ...
    // Per-line taxable + tax, from the discounted net.
    ...
    // The tax already inside the prices, kept apart from the tax added on top.
    ...
    // The grand total is built from the lines and charges themselves, not from subtotal −
    ...
    // Round-off, on the grand total ONLY.
    let round_off = before_round_off.round_adjustment(input.rounding);

    // Round-off recorded as its own figure.
...
mod tests {
    ...
    /// Rates as data, not as constants in the shipping code.
    ...
    /// Both reconciliation identities. Called by nearly every test here, because a bill that
    /// does not reconcile cannot be printed honestly.
    ...
    fn a_composition_bill_has_a_taxable_value_and_no_gst() {
        ...
        let bill =
            compute_bill(BillInput::new(&cart, Registration::Composition)).expect("computes");

        assert!(
            bill.total_gst.is_zero(),
        ...
        assert_eq!(
            bill.grand_total,
            rs(1_000),
            "the customer pays the menu price"
        );
    }
    ...
    fn a_bill_with_food_and_beer_keeps_the_two_taxes_apart() {
        ...
        assert_eq!(
            bill.vat_included, bill.total_vat,
            "an inclusive beer adds nothing on top"
        );
        assert_eq!(
            bill.non_gst_value.add(bill.total_vat.into_money()),
            Ok(rs(250))
        );
        assert_reconciles(&bill);
    }

    /// A deterministic generator, written here rather than pulled in as a dependency.
    struct Rng(u64);
    ...
    fn every_generated_bill_reconciles_both_ways() {
        ...
        for case in 0..3_000_u32 {
            ...
            for line in 0..line_count {
                ...
                // A quantity from 0.001 to ~20, so fractional weights are well represented and
                // not just an occasional half.
            ...
            // Zero to three charges, each with its own rate and treatment.
            ...
            for slot in 0..rng.below(4) {
                ...
                charges.push(Charge {
                    ...
                    basis,
                    // A charge is never alcohol, so its spec is drawn from the GST-side shapes
                    // only — see `Charge::is_coherent`.
                    tax: [
            ...
            // Composition and unregistered shops go through the whole pipeline too, not just
            // `compute_line`.
            let registration = [
            ...
            if !registration.charges_gst() {
                assert!(
                    bill.total_gst.is_zero(),
                    "case {case}: GST on a {registration:?} bill"
                );
            }
            ...
            for line in &bill.lines {
                assert!(
                    !line.net.is_negative(),
                    "case {case} produced a negative line"
                );
                assert!(!line.gross_including_tax.is_negative());
            }
            // Nor may a charge — a charge is never discounted, so a negative one would mean the
            // base leaked a discount into it.
            for charge in &bill.charges {
                assert!(
                    !charge.amount.is_negative(),
                    "case {case} produced a negative charge"
                );
            }
    ...
    fn a_bill_discount_on_a_mixed_rate_bill_taxes_correctly() {
        ...
        // After 10% off, each line's tax must be exactly 10% lower — because the discount came
        // off the net BEFORE tax, per rate.
        ...
        // The regression guard: discounting AFTER tax gives a different total, and it is the
        // wrong one.
    ...
    fn a_service_charge_carries_its_own_rate_into_the_summary() {
        ...
        assert_eq!(
            bill.charges[0].gst.total(),
            Ok(rs(18)),
            "the charge is 18%, not 5%"
        );
    ...
    fn the_charge_base_is_the_discounted_line_total_and_never_compounds() {
        ...
        // Base is ₹900 — the DISCOUNTED line total.
        assert_eq!(bill.charges[0].amount, rs(45), "5% of ₹900, not of ₹1,000");
        // And the second charge is on the same ₹900 — it does NOT compound onto the first
        // (which would give ₹47.25).
    ...
    fn the_charge_base_includes_alcohol() {
        // Service applies to the whole table, non-GST lines included.
        let mut cart = Cart::new();
        ...
        assert_eq!(
            bill.charges[0].amount,
            rs(100),
            "10% of the whole ₹1,000 table"
        );
        assert_reconciles(&bill);
    }
    ...
    fn a_discount_bigger_than_the_bill_cannot_go_negative_even_with_charges() {
        // The naive implementation subtracts the discount from the total AFTER charges and
        // lands below zero.
        let cart = one_line(item("dish", 30_000, pc(5)), Qty::ONE);
        let charges = vec![Charge::flat(
            ChargeKind::Delivery,
            "Delivery",
            rs(40),
            pc(18),
        ...
        let bill = compute_bill(
            input(&cart)
                ...
                .with_rounding(RoundingMode::None)
                .with_bill_discount(DiscountEntry::new(
                    Discount::amount(rs(500)).expect("valid"),
                )),
        )
        .expect("computes");

        assert_eq!(
            bill.total_bill_discount,
            rs(300),
            "it can only take the lines"
        );
        assert!(bill.bill_discount_capped);
        assert_eq!(bill.lines[0].net, Money::ZERO);
        // The delivery charge is still owed — a discount on the food does not pay for the
        // rider.
    ...
    fn a_charge_on_an_empty_bill_computes_rather_than_panics() {
        // A delivery charge with no items is a mistake upstream — but it must produce a bill,
        // not a crash.
        let cart = Cart::new();
        let charges = vec![
        ...
        assert_eq!(
            bill.charges[1].amount,
            rs(40),
            "a flat charge still applies"
        );
        assert_eq!(bill.grand_total, Money::from_paise(4_720));
        assert_reconciles(&bill);
    }
    ...
    fn a_percentage_and_a_flat_discount_that_should_match_do_match() {
        // To the paisa, through the whole pipeline.
        ...
        let by_amount = compute_bill(
            input(&cart)
                .with_rounding(RoundingMode::None)
                .with_bill_discount(DiscountEntry::new(
                    Discount::amount(rs(100)).expect("valid"),
                )),
        )
        .expect("computes");

        assert_eq!(
            by_percent.total_bill_discount,
            by_amount.total_bill_discount
        );
    ...
    fn a_line_discount_carries_its_reason_and_who_gave_it() {
        // The metadata rides with the discount and does not touch the arithmetic.
        let mut cart = Cart::new();
        let index = cart
        ...
        assert_eq!(
            stored.authorised_by.as_ref().map(StaffId::as_str),
            Some("stf_owner")
        );
    ...
    fn an_all_complimentary_bill_with_a_discount_does_not_divide_by_zero() {
        ...
        let bill = compute_bill(input(&cart).with_bill_discount(DiscountEntry::new(
            Discount::percent_bp(1_000).expect("valid"),
        )))
        .expect("computes");
    ...
    fn a_bar_bill_keeps_alcohol_out_of_the_gst_return() {
        let mut cart = Cart::new();
        cart.add(
            item("beer", 30_000, TaxRate::ZERO).with_tax(TaxSpec::liquor(TaxRate::ZERO)),
            Qty::ONE,
            None,
            vec![],
        )
        ...
        cart.add(item("curry", 20_000, pc(5)), Qty::ONE, None, vec![])
            .expect("adds");
        cart.add(
            item("bread", 5_000, TaxRate::ZERO).with_tax(TaxSpec::exempt()),
            Qty::ONE,
            None,
            vec![],
        )
        ...
        assert_eq!(
            bill.summary.rows().count(),
            1,
            "only the 5% dish is a GST rate"
        );
    ...
    fn an_inclusive_line_never_drifts_from_its_menu_price() {
        // Proved end to end, not just inside tax.rs.
        for paise in [10_500_i64, 9_999, 1, 33_333, 100] {
            ...
            let bill =
                compute_bill(input(&cart).with_rounding(RoundingMode::None)).expect("computes");
            assert_eq!(
    ...
    fn a_capped_bill_discount_reaches_the_bill() {
        ...
        assert_eq!(
            bill.total_bill_discount,
            rs(300),
            "it can only take the whole line"
        );
        assert!(
            bill.bill_discount_capped,
            "a silent cap is the bug D7 prevents"
        );
        assert_eq!(bill.grand_total, Money::ZERO);
        assert_reconciles(&bill);
    }
    ...
    fn modifiers_multiply_with_the_quantity_and_inherit_the_rate() {
        ...
        let bill = compute_bill(input(&cart).with_rounding(RoundingMode::None)).expect("computes");
        // (₹200 + ₹30) × 2 = ₹460, taxed at the line's 5%, not at some rate of the modifier's
        // own.
    ...
    fn round_off_reaches_the_rupee_and_is_recorded_separately() {
        ...
        let rounded =
            compute_bill(input(&cart).with_rounding(RoundingMode::NearestRupee)).expect("computes");
    ...
    fn an_absurd_quantity_is_an_error_not_a_wrapped_total() {
        // Nothing panics, the error travels out of compute_bill.
        let cart = one_line(
            item("dish", 100_000, pc(5)),
            Qty::from_thousandths(i64::MAX),
        );
        assert!(matches!(
            compute_bill(input(&cart)),
            Err(BillError::Money(_))
        ));
    }
    ...
    fn both_discounts_apply_in_the_right_order() {
        // Line discount first, then the bill discount on what is left.
        let mut cart = Cart::new();
        let index = cart
            .add(item("dish", 100_000, pc(5)), Qty::ONE, None, vec![])
            .expect("adds");
