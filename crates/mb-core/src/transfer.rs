...
/// One line, and how much of it to move.
...
/// An order's two moving parts, together, because they must never be moved apart.
...
/// Take lines off one order and hand them back as another — the line half of a split.
...
impl Portion {
    /// What the kitchen has still to be told about this portion — the same question
    /// `KitchenLedger::pending` answers, asked of the pair so a caller cannot accidentally ask
    /// it of the wrong cart.
    pub fn pending(&self) -> std::result::Result<Vec<(LineIdentity, Qty)>, QtyError> {
        self.kitchen.pending(&self.cart)
    }

    /// The quantity on every line, added up.
...
/// So a caller can put the pieces back where they came from without naming the fields in four
/// places.
impl Portion {
    #[must_use]
...
/// A line with its quantity, for a caller that wants to move a whole line and does not want to
/// look its quantity up first.
#[must_use]
...
mod tests {
    ...
    fn splitting_a_partly_told_line_leaves_no_duplicate_and_loses_nothing() {
        ...
        // The origin keeps one dosa, and the kitchen already knows about it — so nothing on the
        // origin is waiting to be cooked.
        ...
        // The new party gets two dosas, one of which the kitchen has not heard about yet.
    ...
    /// Taking everything is a MOVE, and a move must not be done by splitting — it would mint a
    /// new order and abandon a bill number.
    #[test]
    ...
    fn more_than_the_line_has_is_refused() {
        ...
        assert!(
            take_lines(
                ...
                &[Pick {
                    index: 0,
                    qty: Qty::from_whole(5).expect("qty")
                }]
            )
            .is_err()
        );
    ...
    /// The three-dosa case. Table 4 was told two, table 5 was told one; if the merged ledger
    /// does not add up, the kitchen cooks three more.
    #[test]
    ...
    fn a_split_bill_adds_back_to_the_original_rate_by_rate() {
        ...
        // Two dosas at 5% and a bottle of water at 18% — the mixed-rate case, because a
        // single-rate split would prove nothing about the summary.
        let mut cart = Cart::new();
        cart.add(
            snapshot("dosa", 12_000),
        ...
        cart.add(water, Qty::from_whole(3).expect("qty"), None, Vec::new())
            .expect("added");
        ...
        let moved = take_lines(
            ...
            &[Pick {
                index: 1,
                qty: Qty::from_whole(3).expect("qty"),
            }],
        )
        .expect("split");

        // What each side is worth before the discount.
        let bare = |portion: &Portion| {
            compute_bill(BillInput::new(
                &portion.cart,
                crate::tax::Registration::Regular,
            ))
            .expect("a bare bill")
        };
        ...
        // Rate by rate. This is the assertion that matters: a chartered accountant adds the two
        // halves' 5% rows and expects the original's.
        ...
        // And the money a customer actually hands over.
