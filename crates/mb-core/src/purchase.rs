...
impl fmt::Display for PurchaseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PurchaseError::Negative { what } => {
                write!(f, "The {what} cannot be a negative amount.")
            }
...
pub struct Entry {
    ...
    pub typed_qty: Qty,
    /// The scheme's free quantity, in the same pack.
    pub free_typed_qty: Qty,
    /// The pack the quantity and the rate are both quoted in — the material's own bag, tin or
    /// tray, or its base unit.
    pub pack: Pack,
    /// Paise per whole `pack`: ₹1,000 a bag is `100000`.
    pub rate: Money,
    /// A discount printed against this line.
    pub discount: Money,
    /// The GST rate on the line, in basis points.
    pub tax_rate_bp: u32,
}
...
pub struct Invoice {
    ...
    /// Transport, loading, hamali — apportioned across the lines by value and becoming part of
    /// the cost of the food, which is the entire point of recording them.
    pub charges: Money,
    /// A property of the SHOP, not of the line.
    pub tax_is_creditable: bool,
    /// Applied to the grand total only, exactly as D4 step 7 applies it to a bill.
...
pub struct Costed {
    ...
    /// What actually came off the vehicle — charged plus free.
    pub received_base_qty: Qty,
    /// Rate × charged quantity.
    pub line_value: Money,
    /// This line's share of the whole-invoice discount.
    pub discount_share: Money,
    /// This line's share of the transport and loading.
    pub charge_share: Money,
    /// The taxable value: line value less both discounts.
    pub taxable: Money,
    pub tax_amount: Money,
    /// The tax that is a cost rather than a credit — zero for a claiming shop, the whole of it
    /// for a 5%-scheme shop.
    pub tax_in_cost: Money,
    pub landed_value: Money,
    /// Paise per 1,000 base units, the same scale as `materials.avg_cost`, so ₹40 a kilo reads
    /// `4000`.
...
pub struct CostedInvoice {
    ...
    pub taxable: Money,
    pub tax_total: Money,
    /// How much of `tax_total` the shop may actually claim back.
...
impl CostedInvoice {
    /// What the shop pays for goods and carriage, before tax.
...
pub fn cost_invoice(invoice: &Invoice) -> Result<CostedInvoice> {
    ...
    if invoice.invoice_discount.is_negative() {
        return Err(PurchaseError::Negative {
            what: "invoice discount",
        });
    }
    if invoice.charges.is_negative() {
        return Err(PurchaseError::Negative {
            what: "transport or loading charge",
        });
    }
    ...
    for (seq, entry) in invoice.lines.iter().enumerate() {
        ...
        // The rate is per WHOLE pack and the quantity is in packs, so this is `Qty::extend`.
        let value = entry.typed_qty.extend(entry.rate)?;
        if entry.discount > value {
            return Err(PurchaseError::DiscountTooBig {
                seq,
    ...
    for (index, entry) in invoice.lines.iter().enumerate() {
        ...
        let tax_in_cost = if invoice.tax_is_creditable {
            Money::ZERO
        } else {
            tax_amount
        };
...
mod tests {
    ...
    fn two_bags_of_rice_at_a_thousand_a_bag() {
        // 2 bags × ₹1,000 = ₹2,000 for 50,000 g, so a kilo cost ₹40 and the stored figure —
        // paise per 1,000 base units — is 4000.
        let costed = cost_invoice(&plain(vec![line(2_000, 0, 100_000, bag())])).expect("costs");
        assert_eq!(
            costed.lines[0].received_base_qty,
            Qty::from_thousandths(50_000_000)
        );
        assert_eq!(costed.total, Money::from_paise(200_000));
        assert_eq!(
            costed.lines[0].landed_unit_cost,
    ...
    fn a_free_bag_is_a_denominator_and_not_a_zero_priced_line() {
        // 10 bags at ₹1,000 with 1 free: ₹10,000 bought 11 bags, so a bag cost ₹909.0909… and a
        // kilo cost ₹36.3636… — 3636 paise per 1,000 g after one rounding, and NOT the ₹40 a
        // naive reading gives.
        let costed =
            cost_invoice(&plain(vec![line(10_000, 1_000, 100_000, bag())])).expect("costs");
        assert_eq!(
            costed.lines[0].received_base_qty,
            Qty::from_thousandths(275_000_000)
        );
        ...
        // The shelf gains eleven bags, not ten.
        assert_eq!(
    ...
    fn transport_is_apportioned_by_value_and_the_parts_sum_exactly() {
        // ₹200 of tempo across three awkward lines.
        let invoice = Invoice {
            lines: vec![
        ...
        // And the charge really is inside the cost: the landed value of every line is more than
        // its taxable value.
    ...
    fn a_line_discount_lowers_the_cost_and_an_impossible_one_is_refused() {
        ...
        assert!(matches!(
            refused,
            Err(PurchaseError::DiscountTooBig { seq: 0, .. })
        ));
    ...
    fn the_round_off_lands_on_the_grand_total_only() {
        ...
        let costed = cost_invoice(&invoice).expect("costs");
        assert_eq!(
            costed.total.paise() % 100,
            0,
            "a rounded total ends in whole rupees"
        );
    ...
    fn everything_still_reconciles_when_all_four_happen_at_once() {
        // A real mandi invoice: two materials, a free bag, a line discount, an invoice
        // discount, transport, and tax the shop cannot claim.
        let invoice = Invoice {
            lines: vec![
                Entry {
        ...
        // And the money in the lines is the money on the invoice: every paisa of goods, charge
        // and non-creditable tax is inside some line's landed value, which is what makes the
        // stock valuation reconcile with the supplier ledger.
