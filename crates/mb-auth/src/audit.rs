...
/// Every action this build can write.
pub mod action {
    ...
    /// Something a phone asked the counter to do.
    ...
    /// A licensing action that was refused — a wrong emergency code, a transfer inside its
    /// cooldown.
    ...
    /// The balance cache was rebuilt from the ledger.
    ...
    /// The only correction path a purchase has, and therefore the row somebody will be asked
    /// about.
    ...
    /// The one row of the leave ledger a person writes freehand — and therefore the one
    /// somebody could use to invent a fortnight's holiday.
...
pub struct AuditEntry {
    ...
    /// `None` for a failed login against a name nobody could match — there is genuinely no
    /// staff member, and inventing one would be a lie in the one table that must not contain
    /// any.
    pub staff_id: Option<StaffId>,
    pub action: AuditAction,
    pub entity: &'static str,
    pub entity_id: Option<String>,
    /// The ONE place JSON is allowed in this product (migration 0001 says so): the shape
    /// differs per action and nothing ever queries inside it.
...
/// The link. Every field that a person could want to change is in it — if a column is not
/// hashed, editing that column is undetectable, so the only safe default is "all of them".
#[must_use]
...
fn hex(bytes: &[u8]) -> String {
    ...
    bytes.iter().fold(String::with_capacity(64), |mut s, b| {
        // The only way this write fails is an allocation failure, which is not a case a counter
...
pub enum BrokenWhy {
    ...
    Edited,
    /// This row does not point at the previous one: something was removed or reordered between
    /// them.
    Unlinked,
...
mod tests {
    ...
    fn a_rewritten_row_with_a_recomputed_hash_still_breaks_the_next_link() {
        // The interesting attack: somebody who knows how the hash is made changes a row AND
        // fixes its hash.
    ...
    fn correcting_a_staff_members_spelling_does_not_break_history() {
        // Staff_name is a join, not a hashed column.
