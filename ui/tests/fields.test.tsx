/**
 * **The two fields that have a shape** — the owner, 2026-08-22.
 *
 ...
 * > calculations also… also make sure when you add rupee symbol, it does not
 * > interfere with calculations (do not save rupee symbol also with amount to
 * > db, it would be problem)."*
 *
 * `check-fields.mjs` proves every phone and amount box in the product IS one of
 * these. This proves the components hold the rule.
 */
...
describe('an amount is digits and one dot', () => {
  it('drops letters as they are typed, rather than complaining afterwards', () => {
    // A field that lets you type "12ab" and refuses on save has already wasted
    // the keystroke and the trip to Rust.
  ...
  it('allows one dot and two paise, which is what Money::parse takes', () => {
    ...
    // A third place is `MoneyError::TooPrecise` in Rust. Stopping it here means
  ...
  it('lets an amount be cleared, because empty is not zero', () => {
    // The owner: *"no need for 0.00, just keep it empty."* An amount nobody has
    // typed is nothing yet, and a field that cannot be emptied is a field that
    // cannot be corrected.
  ...
  it('does not announce the mark to a screen reader', () => {
    // The label already says it is an amount; "rupees four fifty" before every
    // field is noise.
    const { container } = render(
      <MoneyInput label="Amount" value="450" onChange={vi.fn()} />,
    );
...
describe('a phone is ten digits', () => {
  ...
  it('stops at ten', () => {
    // The owner: *"i can enter alphabets, more than 10 numbers."*
  ...
  it('understands a number pasted out of a contact list', () => {
    // Refusing a paste is how you make somebody retype what the program could
    // have understood.
